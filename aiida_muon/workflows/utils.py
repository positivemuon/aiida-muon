...
def get_collinear_mag_kindname(p_st,magm):
    ...
    for i, m in enumerate(coll_m):
        ...
        p_st[i].properties['magmom'] = Magmom([0.,0.,mtm[2]])
...
def check_get_hubbard_u_parms(p_st):
    ...
    """
    #materials_project
    U_dict1={"Co":3.32, "Cr":3.7 , "Fe":5.3, "Mn":3.9, "Mo":4.38, "Ni":6.2, "V":3.25, "W":6.2, "Cu": 0.0}
    #PRB 73, 195107 2006
    U_dict2={"Co":3.3,  "Cr":3.5 , "Fe": 4.0, "Mn": 4.0, "Mo":0.0, "Ni":6.4, "V":3.1, "W":0.0, "Cu": 4.0}
        ...
        for j,pj in enumerate(frac_positions):
            if j > i:
                ...
                if (energies is not None) and (len(energies) == len(frac_positions)):
                    if (np.linalg.norm(diff, axis=0) < min_distance) and (abs(energies[i]- energies[j]) < e_tol):
                        s_idx[j]=-1
                else:
