...
class FindMuonWorkChain(ProtocolMixin, WorkChain):
    ...
    def converge_supercell(self):
        ...
        if not self.inputs.hubbard_u: 
            inputs.structure = orm.StructureData(pymatgen = self.inputs.structure) # so we lose the info on hubbard. this is the case where we use protocol but then we set builder.hubbard = False later.
